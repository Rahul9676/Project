...
# ----------------------------------------------------------------------------
def find_data_file():
    """Locate epa_air_quality.csv whether it sits in data/, at the repo root, or a subfolder."""
    here = os.path.dirname(os.path.abspath(__file__))
    roots = [here, os.getcwd()]
    names = ["data/epa_air_quality.csv", "epa_air_quality.csv"]
    for r in roots:
        for n in names:
            p = os.path.join(r, n)
            if os.path.exists(p):
                return p
    # last resort: walk the tree for the file by name (any folder/casing on the stem)
    for r in roots:
        for dirpath, _dirs, files in os.walk(r):
            for f in files:
                if f.lower() == "epa_air_quality.csv":
...
if up is not None:
    ...
    st.error(
        "**Couldn't find `epa_air_quality.csv`.** Add it to your repo — either at the repo root "
        "next to `app.py`, or in a `data/` subfolder — then redeploy. You can also upload it now "
        "with the **Replace data** control in the sidebar."
    )
    st.stop()
